...
class JsonToken {
  constructor({
    ...
    this.series_id = seriesId; //token ID
...
class JsonSeries {
  constructor({
    ...
    //series ID
    this.series_id = seriesId, //token metadata
    this.metadata = metadata, //owner of the token
    this.owner_id = ownerId, //keep track of the royalty percentages for the token in a hash map
    this.royalty = royalty;
  }
...
class Series {
  constructor({
    ...
    this.metadata = metadata, //owner of the token
    this.owner_id = ownerId, //keep track of the royalty percentages for the token in a hash map
    this.royalty = royalty, //list of tokens in the series
    this.tokens = tokens;
  }
...
function internalCreateSeries({
  ...
  contract.seriesById.set(id, series); //calculate the required storage which was the used - initial TODO

  let requiredStorageInBytes = storageUsage().valueOf() - initialStorageUsage.valueOf(); //refund any excess storage if the user attached too much. Panic if they didn't attach enough to cover the required.

  refundDeposit(requiredStorageInBytes);
}
function internalUpdateSeriesId({
  contract,
  currentId,
  newId
}) {
  let caller = predecessorAccountId(); // @ts-ignore

  let series = contract.seriesById.get(currentId);

  if (series == null) {
    panic("no series");
  }

  assert(series.owner_id == caller, "Not owner"); // @ts-ignore

  assert(contract.seriesById.get(newId) == null, "New Series already exists"); // @ts-ignore

  contract.seriesById.remove(currentId); // @ts-ignore
...
function internalNftToken({
  ...
  let curSeries = contract.seriesById.get(token.series_id);
...
function internalNftTokens({
  ...
  let keys = contract.tokensById.toArray(); // Paginate through the keys using the fromIndex and limit
...
function internalSeries({
  ...
  let max = limit ? limit : 50;
  let keys = contract.seriesById.toArray(); // Paginate through the keys using the fromIndex and limit

  for (let i = start; i < keys.length && i < start + max; i++) {
    // get the token object from the keys
    // @ts-ignore
    let jsonToken = internalSeriesInfo({
      contract,
      id: keys[i][0]
    });
    tokens.push(jsonToken);
...
function internalSeriesInfo({
  contract,
  id
}) {
  // @ts-ignore
  let series = contract.seriesById.get(id); //if there wasn't a series in the seriesById collection, we return None
...
function internalNftSupplyForSeries({
  contract,
  id
}) {
  // @ts-ignore
...
function internalNftPayout({
  ...
  let royalty = curSeries.royalty; //make sure we're not paying out to too many people (GAS limits this)
...
function internalAddApprovedMinters({
  contract,
  accountId
}) {
  // Assert the predecessor is the current account ID
  const predecessorAccountId$1 = predecessorAccountId();
  assert(predecessorAccountId$1 === currentAccountId(), "Only the current account can add approved minters");
  contract.approvedMinters.set(accountId);
}
function internalRemoveApprovedMinters({
  contract,
  accountId
}) {
  // Assert the predecessor is the current account ID
  const predecessorAccountId$1 = predecessorAccountId();
  assert(predecessorAccountId$1 === currentAccountId(), "Only the current account can remove approved minters");
  contract.approvedMinters.remove(accountId);
}
function internalIsApprovedMinter({
  contract,
  accountId
}) {
  return contract.approvedMinters.contains(accountId);
}
function internalAddApprovedCreator({
  contract,
  accountId
}) {
  // Assert the predecessor is the current account ID
  const predecessorAccountId$1 = predecessorAccountId();
  assert(predecessorAccountId$1 === currentAccountId(), "Only the current account can add approved creators");
  contract.approvedCreators.set(accountId);
}
function internalRemoveApprovedCreator({
  contract,
  accountId
}) {
  // Assert the predecessor is the current account ID
  const predecessorAccountId$1 = predecessorAccountId();
  assert(predecessorAccountId$1 === currentAccountId(), "Only the current account can remove approved creators");
  contract.approvedCreators.remove(accountId);
}
function internalIsApprovedCreator({
  contract,
  accountId
}) {
...
let Contract = NearBindgen(_class = (_class2 = class Contract extends NearContract {
  ...
  */
  ...
  nft_supply_for_series({
    ...
    return internalNftSupplyForSeries({
      contract: this,
      id: id
    });
  }
  ...
  nft_tokens_for_series({
    ...
    return internalNftTokensForSeries({
      contract: this,
      id: id,
      fromIndex: from_index,
      limit: limit
  ...
  */
  ...
  add_approved_minters({
    account_id
  }) {
    return internalAddApprovedMinters({
      contract: this,
      accountId: account_id
    });
  }

  remove_approved_minters({
    account_id
  }) {
    return internalRemoveApprovedMinters({
      contract: this,
      accountId: account_id
    });
  }
  ...
  is_approved_minter({
    ...
    return internalIsApprovedMinter({
      contract: this,
      accountId: account_id
    });
  }
  ...
  add_approved_creator({
    ...
    return internalAddApprovedCreator({
      contract: this,
      accountId: account_id
    });
  }
  ...
  remove_approved_creator({
    ...
    return internalRemoveApprovedCreator({
      contract: this,
      accountId: account_id
    });
  }
  ...
  is_approved_creator({
    ...
    return internalIsApprovedCreator({
      contract: this,
      accountId: account_id
    });
  }

}, (_applyDecoratedDescriptor(_class2.prototype, "create_series", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "create_series"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_mint", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_mint"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "update_series_id", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "update_series_id"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_token", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_token"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_transfer", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_transfer"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_transfer_call", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_transfer_call"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_resolve_transfer", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_resolve_transfer"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_is_approved", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_is_approved"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_approve", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_approve"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_payout", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_payout"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_transfer_payout", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_transfer_payout"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_revoke", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_revoke"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_revoke_all", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_revoke_all"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_total_supply", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_total_supply"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_tokens", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_tokens"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_tokens_for_owner", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_tokens_for_owner"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_supply_for_owner", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_supply_for_owner"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_supply_series", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "get_supply_series"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_series", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "get_series"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_series_info", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "get_series_info"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_supply_for_series", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_supply_for_series"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_tokens_for_series", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_tokens_for_series"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "nft_metadata", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_metadata"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "add_approved_minters", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "add_approved_minters"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "remove_approved_minters", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "remove_approved_minters"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "is_approved_minter", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "is_approved_minter"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "add_approved_creator", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "add_approved_creator"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "remove_approved_creator", [call], Object.getOwnPropertyDescriptor(_class2.prototype, "remove_approved_creator"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "is_approved_creator", [view], Object.getOwnPropertyDescriptor(_class2.prototype, "is_approved_creator"), _class2.prototype)), _class2)) || _class;
function init() {
  Contract._init();
}
function is_approved_creator() {
...
function remove_approved_creator() {
  ...
  let ret = _contract.remove_approved_creator(args);

  _contract.serialize();

  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function add_approved_creator() {
  let _contract = Contract._get();
  ...
  let ret = _contract.add_approved_creator(args);

  _contract.serialize();

  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function is_approved_minter() {
  let _contract = Contract._get();
  ...
  let ret = _contract.is_approved_minter(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function remove_approved_minters() {
  let _contract = Contract._get();
  ...
  let ret = _contract.remove_approved_minters(args);

  _contract.serialize();

  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function add_approved_minters() {
  let _contract = Contract._get();
  ...
  let ret = _contract.add_approved_minters(args);
...
function nft_tokens_for_series() {
  let _contract = Contract._get();
  ...
  let ret = _contract.nft_tokens_for_series(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function nft_supply_for_series() {
  let _contract = Contract._get();
  ...
  let ret = _contract.nft_supply_for_series(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function get_series_info() {
  ...
  let ret = _contract.get_series_info(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function get_series() {
  let _contract = Contract._get();
  ...
  let ret = _contract.get_series(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
function get_supply_series() {
  let _contract = Contract._get();
  ...
  let ret = _contract.get_supply_series(args);
  if (ret !== undefined) env.value_return(_contract.constructor.serializeReturn(ret));
}
...
function update_series_id() {
  ...
  let ret = _contract.update_series_id(args);
...
function create_series() {
  ...
  let ret = _contract.create_series(args);
...
export { Contract, NFT_METADATA_SPEC, NFT_STANDARD_NAME, add_approved_creator, add_approved_minters, create_series, get_series, get_series_info, get_supply_series, init, is_approved_creator, is_approved_minter, nft_approve, nft_is_approved, nft_metadata, nft_mint, nft_payout, nft_resolve_transfer, nft_revoke, nft_revoke_all, nft_supply_for_owner, nft_supply_for_series, nft_token, nft_tokens, nft_tokens_for_owner, nft_tokens_for_series, nft_total_supply, nft_transfer, nft_transfer_call, nft_transfer_payout, remove_approved_creator, remove_approved_minters, update_series_id };
